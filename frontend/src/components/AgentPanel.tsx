  ...
  return (
    <section className="panel agent">
      ...
      <p className="muted small">
        Describe what you want in plain language. A router agent decides which tool fits, the tool runs, and a verifier
        agent checks the result against your documents – every step is shown below.
        {selectedDocs.length > 0 && <> Using: {selectedDocs.map((d) => d.display_name).join(', ')}.</>}
      </p>
      ...
      <div className="composer">
        ...
        <div className="composer-row">
          <label className="row gap-s small"><input type="checkbox" checked={autoVerify} onChange={(e) => setAutoVerify(e.target.checked)} />verify answers</label>
          <label className="row gap-s small"><input type="checkbox" checked={allowFiles} onChange={(e) => setAllowFiles(e.target.checked)} />allow file generation</label>
          <span className="grow" />
          ...
          {steps.map((s, i) => (
            <li key={i} className={`trace-step ${s.status}`}>
              <div className="row between">
                ...
                <span className={`badge ${s.status === 'done' ? 'green' : s.status === 'error' ? 'red' : 'gray'}`}>{s.status}</span>
              </div>
